...
7. Pour un PDF-image, Tesseract exécute deux segmentations complémentaires. `PSM.SINGLE_BLOCK` préserve l'ossature du tableau ; `PSM.AUTO` retrouve les blocs et continuations de texte complexes.
8. Les deux résultats sont alignés par code article. Le libellé exploitable le plus complet est retenu, tandis qu'un fragment OCR sans code n'est jamais autorisé à créer un produit supplémentaire.
9. L'appartenance des mots aux lignes fournie par Tesseract est conservée. La proximité verticale, calculée depuis la hauteur réelle des glyphes, ne sert que de repli lorsque cette information n'existe pas.
10. Une unité de stockage vide peut être reconstruite à partir d'une quantité explicite dans l'article (`kg`, `g`, `L`, `cl`, `ml`). Une valeur non ambiguë comme `au Kg` ou `au L`, décalée dans le conditionnement, est replacée dans la colonne stockage.
11. Le bilan reste visible après l'import : mode choisi (`texte`, `OCR` ou `texte + OCR`), codes détectés, lignes restituées et éventuels contrôles manuels.
...
- segmentation automatique qui transforme un code ou un morceau de marque en ligne produit séparée.
