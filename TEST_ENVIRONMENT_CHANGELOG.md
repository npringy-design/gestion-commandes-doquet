...
- Sauvegarde granulaire de `Calcul vente ratio` préparée pour TEST à la suite d'un échec de sauvegarde : l'ancien bloc global `products` reste lisible mais n'est plus réécrit. Chaque modification persiste uniquement `ratioProduct:<id>`, une suppression utilise un marqueur ciblé et le glisser-déposer ne sauvegarde que `ratioProductOrder`. La table `app_state`, la file locale fiable et le contrôle last-write-wins existants sont réutilisés sans migration SQL. Les produits non encore modifiés restent issus de l'ancien bloc, ce qui évite toute réécriture massive ou perte pendant la transition. Les autres pages restent inchangées et seront traitées séparément après validation TEST.
- Déplacement des produits dans `Calcul vente ratio` préparé pour TEST : les flèches haut/bas sont remplacées par un glisser-déposer de toute zone libre de la carte avec maintien du clic gauche. Le dépôt sur une autre carte modifie l'ordre dans la grille, y compris en deux colonnes, puis réutilise la sauvegarde automatique existante. Les champs et boutons restent interactifs, les produits masqués et les autres fournisseurs ne sont pas déplacés, et aucun ratio, mois figé ou historique n'est modifié.
- Correctif préparé pour TEST après comparaison de TEST et production : lorsqu'une nouvelle ligne de trame retrouve par son nom un produit déjà créé lors d'un essai précédent mais toujours masqué par les mois figés, l'action `Créer les nouveaux produits` ne le duplique pas et ouvre ses douze mois pour lui seul. Cela couvre notamment un produit créé en production avant l'ajout de l'ouverture automatique ; les autres produits restent figés.
