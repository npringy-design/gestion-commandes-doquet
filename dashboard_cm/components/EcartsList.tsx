...
const EcartsList: React.FC<EcartsListProps> = ({ title, items, type, onSelectItem, selectedId, periodSales }) => {
  ...
  return (
    <div className="bg-white/70 backdrop-blur rounded-2xl shadow-sm border border-white/60 overflow-hidden flex flex-col h-full min-h-0">
          ...
          <tbody className="divide-y divide-slate-50">
            {items.map((item, idx) => (
              ...
              >
                ...
                <td className={`hidden sm:table-cell px-3 py-2 text-right tabular-nums ${item.quantity > 0 ? 'text-rose-500' : item.quantity < 0 ? 'text-emerald-500' : 'text-slate-500'}`}>
                  {item.quantity.toLocaleString('fr-FR', { maximumFractionDigits: 2 })}
                </td>
                <td className={`px-2 sm:px-3 py-2 text-right font-bold tabular-nums ${item.value > 0 ? 'text-rose-600' : item.value < 0 ? 'text-emerald-600' : 'text-slate-900'}`}>
                  {item.value.toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} €
                </td>
                <td className="hidden lg:table-cell px-3 py-2 text-right text-slate-700 tabular-nums font-semibold">
