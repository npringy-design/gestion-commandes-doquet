...
export const buildSiteScopedCloudKey = (siteId: string, key: string): string => `site:${siteId}:${key}`;

export const loadSiteStateFromSupabase = async (siteId: string): Promise<Array<SupabaseRow> | null> => {
  if (!isSupabaseConfigured()) return null;
  try {
    const prefix = encodeURIComponent(`site:${siteId}:`);
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/${TABLE}?select=key,value,updated_at&key=like.${prefix}%25`,
      { headers: headers() }
    );
    if (!res.ok) return null;
    return await res.json() as SupabaseRow[];
...
export const loadSiteMetaFromSupabase = async (siteId: string): Promise<Array<Pick<SupabaseRow, 'key' | 'updated_at'>> | null> => {
  if (!isSupabaseConfigured()) return null;
  try {
    const prefix = encodeURIComponent(`site:${siteId}:`);
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/${TABLE}?select=key,updated_at&key=like.${prefix}%25`,
      { headers: headers() }
    );
    if (!res.ok) return null;
    return await res.json() as Array<Pick<SupabaseRow, 'key' | 'updated_at'>>;
...
export const loadSiteKeysFromSupabase = async (siteId: string, keys: string[]): Promise<SupabaseRow[] | null> => {
  if (!isSupabaseConfigured()) return null;
  if (!keys || keys.length === 0) return [];
  try {
    const encoded = keys
      .map(k => `"${String(buildSiteScopedCloudKey(siteId, k)).replace(/"/g, '\\"')}"`)
      .join(',');
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/${TABLE}?select=key,value,updated_at&key=in.(${encoded})`,
      { headers: headers() }
