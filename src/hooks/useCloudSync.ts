  ...
  const persistEverywhere = useCallback((key: string, value: unknown) => {
    ...
    saveToSupabaseDebounced(
      ...
      (confirmedKey, confirmedTs) => {
        // confirmedTs = timestamp SERVEUR réel (après trigger Postgres)
        // On synchronise notre curseur cloud avec la valeur réelle en base.
        // Ainsi les comparaisons LWW futures utilisent des timestamps cohérents.
        lastCloudUpdatedAtByKey.current[confirmedKey] = confirmedTs;
        // On garde localTsByKey avec confirmedTs pour que le Realtime entrant
        // (qui arrive avec le même timestamp serveur) soit correctement ignoré
        // comme étant notre propre écriture.
        localTsByKey.current[confirmedKey] = confirmedTs;
        // Nettoyage différé pour laisser le temps au Realtime d'arriver
        setTimeout(() => { delete localTsByKey.current[confirmedKey]; }, 3000);
      }
    );
